import pandas as pd
import numpy as np
import os
import glob
from scipy.stats import mannwhitneyu, kstest, brunnermunzel
import matplotlib.pyplot as plt

bigmatrices_directory =  '../../created/big_matrices/'
all_dir = glob.glob(bigmatrices_directory+ "complementarity_3.csv" )
growth_matrix_directory = '../../created/growth/MaxOD_3.csv'


saving_directory_selected_pathways = '../../created/pathway_selection/selected_highStd/'
saving_directory_ranked_pathways = '../../created/pathway_selection/ranked_highStd/'
saving_directory_coef_pathways = '../../created/pathway_selection/estimator_highStd/'

file = "../../extra/high_std_pw_new.csv"
df_file = pd.read_csv(file, sep=",")
df_file.columns = ["pw", "std"]
list_pws = df_file.pw.tolist()
for i in range(len(list_pws)):
    list_pws[i] = list_pws[i].replace("/", "-")


def call_WMW():

    # Perform the process for every interaction index.
    for file_path in sorted(all_dir):

        df_Xy = pd.read_csv(file_path, index_col=0)
        df_Xy = df_Xy.fillna(0)
        name_measure = file_path[len(bigmatrices_directory):-4]
        # df_Xy = pd.concat([df_Xy, df_Xy[df_Xy.y == 1]])#,df_Xy[df_Xy.y == 1]])
        #
        # ------------------------------------------------------------------
        # df_Xy = df_Xy.loc[df_Xy.index.str.split("x").str[1]!="St",]
        # ------------------------------------------------------------------

        # df_copy = df_Xy.copy()
        # donors = df_Xy.index.str.split('x').str[0].str.split('.').str[0]
        # acceptors = df_Xy.index.str.split('x').str[1].str.split('.').str[0]
        # pair = ["{}x{}".format(a,b) for a,b in zip(donors, acceptors)]
        # df_Xy = df_Xy.groupby(pair).mean()



        # df_Xy = df_Xy.groupby(df_Xy.columns.str[0:2], axis=1).mean()
        # Get X (matrix with data) and y (column with labels).
        df_X = df_Xy.loc[:, df_Xy.columns != 'y']
        df_X = df_X[list_pws] # NEW!!!
        # print(df_X.to_numpy().flatten())
        # print(np.repeat(df_Xy.y, df_X.shape[1]))
        # for pathway in list_pws:
        #     # pathway="Folate biosynthesis"
        #     # print(df_X[df_Xy.y==0])
        #     groupA = df_Xy.loc[df_Xy.y ==0, pathway]
        #     groupB = df_Xy.loc[df_Xy.y ==1, pathway]
        #     plt.hist(groupA, density=True, fc=(0, 0, 1, 0.5))
        #     plt.hist(groupB, density=True, fc=(1, 0, 0, 0.5))
        #     # plt.show()
        #     # plt.plot(df_X[pathway].to_numpy().flatten(), df_Xy.y, 'bo')
        #     plt.savefig("extra2/"+pathway+".png")
        #     plt.close()
        # exit()

        # print(df_Xy[df_Xy.y == 0].shape)
        # aa = pd.concat([df_Xy[df_Xy.y == 1], df_Xy[df_Xy.y == 1]])
        # print(aa.shape)
        # results = df_X.apply(lambda d: kstest(d[df_Xy.y == 0],
        #                                       d[df_Xy.y == 1],
        #                                       alternative='greater'),
        #                      axis=0)
        # print(results)
        # exit()
        # Perform the MWU test per column
        try:
            # results = df_X.apply(lambda d: mannwhitneyu(d[df_Xy.y == 0],
            results = df_X.apply(lambda d: mannwhitneyu(d[df_Xy.y == 0],
                                                        d[df_Xy.y == 1],
                                                        alternative='less'),
                                 axis=0)
        except:
            results = pd.DataFrame(index=range(2),columns=range(df_X.shape[1]))
            results.columns = df_X.columns

        df_temp = pd.DataFrame({
            'pathways': df_X.columns.tolist(),
            'pvalues': [x for x in results.values[1]],
            'statistic': [x for x in results.values[0]]})
        # df_temp = df_temp.sort_values(by='pvalues')
        # df_temp = df_temp.reset_index(drop=True)
        # df_temp['rank'] = np.arange(1, len(df_temp) + 1)
 
        # Get Ranked+Selected pathways into DataFrames
        if 'df_ranked_WMW' in locals():
            # df_ranked_temp = pd.DataFrame({
            #     'pathways': df_temp['pathways'].tolist(),
            #     name_measure: df_temp['rank'].tolist()})
            # df_ranked_WMW = pd.merge(df_ranked_WMW, df_ranked_temp, how="outer", on="pathways")
            #
            #
            # df_selected_WMW = pd.concat([
            #     df_selected_WMW,
            #     pd.DataFrame({name_measure: df_temp.iloc[:min(20, len(df_X.columns)), 0]})],
            #     axis=1)

            df_coef_WMW = pd.concat([
                df_coef_WMW,
                pd.DataFrame({name_measure: df_temp['statistic'],
                              'pval':df_temp['pvalues']})],
                axis=1)

        else:
            # df_ranked_WMW = pd.DataFrame({
            #     'pathways': df_temp['pathways'].tolist(),
            #     name_measure: df_temp['rank'].tolist()})
            #
            # df_selected_WMW = pd.DataFrame({
            #     name_measure: df_temp.iloc[:min(20, len(df_X.columns)), 0]})

            df_coef_WMW = pd.DataFrame({
                "pathway": df_X.columns,
                name_measure: df_temp['statistic'],
                'pval':df_temp['pvalues']})

    # Save
    # df_ranked_WMW = df_ranked_WMW.sort_values(by=df_ranked_WMW.columns[1])
    # df_selected_WMW = pd.DataFrame(
    #     np.sort(df_selected_WMW.values, axis=0),
    #     index=df_selected_WMW.index, columns=df_selected_WMW.columns)


    # if not os.path.exists(saving_directory_ranked_pathways):
    #     os.makedirs(saving_directory_ranked_pathways)
    # if not os.path.exists(saving_directory_selected_pathways):
    #     os.makedirs(saving_directory_selected_pathways)
    if not os.path.exists(saving_directory_coef_pathways):
            os.makedirs(saving_directory_coef_pathways)

    # df_ranked_WMW.to_csv(saving_directory_ranked_pathways+'ranked_WMW.csv',
    #                         header=True, index=False)
    # df_selected_WMW.to_csv(saving_directory_selected_pathways+'selected_WMW.csv',
    #                         header=True, index=False)
    df_coef_WMW.to_csv(saving_directory_coef_pathways+'estimator_WMW_2.csv',
                          header=True, index=False)
                          
