...
def call_Boruta():
    ...
    for file_path in sorted(all_dir):
        ...
        if 'df_ranked_Boruta' in locals():
            ...
            df_coef_Boruta = pd.concat([
                df_coef_Boruta,
                pd.DataFrame({name_measure: feat_imp_X,
                              "support": support_X,
                              })],#rf.fit(df_X, df_y).feature_importances_})],
                axis=1)
            ...
            df_coef_Boruta = pd.DataFrame({
                ...
                "support": support_X,
                name_measure: feat_imp_X})#rf.fit(df_X, df_y).feature_importances_})
    ...
    #                         header=True, index=False)
    df_coef_Boruta.to_csv(saving_directory_coef_pathways+'estimator_Boruta_2.csv',
                       header=True, index=False)
