...
# Global variables
source_dir = "~/Documents/GitHub/UTI_bacteria_interactions/src/visualization/util/fig2/temp/KO_binary_matrices/"
...
file = "all.csv"
hm_temp <- make_KO_binary_plots_colourized(file, source_dir)
# Save
# dir.create(file.path(saving_dir), showWarnings = FALSE)
# pdf(paste(saving_dir, pathway_name, saving_format, sep=""))
# draw(hm_temp, column_title = pathway_name)
# dev.off()

# for (file in source_files){
#   hm_temp <- make_KO_binary_plots_colourized()
#   # Save
#   dir.create(file.path(saving_dir), showWarnings = FALSE)
#   pdf(paste(saving_dir, pathway_name, saving_format, sep=""))
#   draw(hm_temp, column_title = pathway_name)
